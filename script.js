...
// --- FIX: Modal always centered and only open on button click ---

// Helper to center modal always
function centerModal(modal) {
    if (window.innerWidth <= 600) {
        ...
        modal.style.alignItems = null;
        modal.style.justifyContent = null;
    }
}

customTimeBtn.addEventListener("click", () => {
    centerModal(customTimeModal);
});
...
resetCustomTimeBtn.addEventListener("click", (event) => {
    event.preventDefault();
    focusTime = 25;
    shortBreakTime = 5;
    longBreakTime = 15;
...
customizeColorsBtn.addEventListener("click", () => {
    centerModal(colorCustomizationModal);
});
...
resetColorsBtn.addEventListener("click", (event) => {
    event.preventDefault();
...
// Remove any auto-open modal on load/resize
window.addEventListener("resize", () => {
    // Do not auto open any modal on resize
    // If modal is open, keep it centered
    if (customTimeModal.style.display !== "none" && customTimeModal.style.display !== "") {
        centerModal(customTimeModal);
    }
    if (colorCustomizationModal.style.display !== "none" && colorCustomizationModal.style.display !== "") {
        centerModal(colorCustomizationModal);
    }
});

// --- FIX: Pastikan event handler utama tidak tertimpa oleh event handler modal ---
// Pindahkan event handler utama ke bawah agar tidak tertimpa
